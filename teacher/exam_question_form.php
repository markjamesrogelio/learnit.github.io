...
if ($question_id) {
    ...
    $stmt = $conn->prepare("SELECT question_type, question_text, answer, options FROM exam_questions WHERE id = ? AND exam_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($question_text) || empty($question_type) || empty($answer)) {
        ...
        if ($question_id) {
            ...
            $stmt = $conn->prepare("UPDATE exam_questions SET question_text = ?, question_type = ?, answer = ?, options = ? WHERE id = ? AND exam_id = ?");
            $stmt->bind_param("ssssii", $question_text, $question_type, $answer, $options, $question_id, $exam_id);
        } else {
            // Create a new question
            $stmt = $conn->prepare("INSERT INTO exam_questions (exam_id, question_text, question_type, answer, options) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("issss", $exam_id, $question_text, $question_type, $answer, $options);
        }

        if ($stmt->execute()) {
            $message = 'Question saved successfully.';
            header("Location: dashboard.php?section=exam&action=questions_list&exam_id=$exam_id&msg=success");
            exit();
        } else {
...
<head>
    <meta charset="UTF-8" />
    <title><?= $question_id ? 'Edit' : 'Add' ?> Exam Question | LearnIT</title>
    <link rel="stylesheet" href="../assets/css/admin_sidebar.css" />
</head>
<body>
    <div class="container" style="padding:20px;">
        <h2><?= $question_id ? 'Edit' : 'Add' ?> Exam Question</h2>
        ...
        <form method="POST">
            ...
            <a href="dashboard.php?section=exam&action=questions_list&exam_id=<?= $exam_id ?>" class="btn btn-cancel">Cancel</a>
        </form>
    </div>
</body>
</html>
